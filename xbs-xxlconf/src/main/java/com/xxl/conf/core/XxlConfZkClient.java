...
public class XxlConfZkClient implements Watcher {
		...
		if (zooKeeper==null) {
			try {
				if (INSTANCE_INIT_LOCK.tryLock(2, TimeUnit.SECONDS)) {
					try {
						System.out.println("####Environment.ZK_ADDRESS####:"+Environment.ZK_ADDRESS);
						...
						XxlConfZkClient.createWithParent(Environment.CONF_CACHE_DATA_PATH);	// init cfg root path
					} finally {
	...
	private static String pathToKey(String nodePath){
		if (nodePath==null || nodePath.length() <= Environment.CONF_CACHE_DATA_PATH.length() || !nodePath.startsWith(Environment.CONF_CACHE_DATA_PATH)) {
			return null;
		}
		return nodePath.substring(Environment.CONF_CACHE_DATA_PATH.length()+1, nodePath.length());
	}
    ...
    public static String keyToPath(String nodeKey){
    	if(nodeKey.indexOf("/")!=-1){
    		...
    		if(null!=spitLists){
    			...
    			return Environment.CONF_CACHE_DATA_PATH + "/" + groupName + "/" + nodeName;
    		}
    	}
        return Environment.CONF_CACHE_DATA_PATH + "/" + Environment.PROJECT_GROUP_NAME + "/" + nodeKey;
    }
    public static String keyGroupToPath(String nodeGroup, String nodeKey) {
        return Environment.CONF_CACHE_DATA_PATH + "/" + nodeGroup + "/" + nodeKey;
    }

    public static String keyGroupToPathByAdmin(String groupKey) {
        return Environment.CONF_CACHE_DATA_PATH + "/" + groupKey;
    }
	...
	public static String getPathDataByKey(String key){
		String path = keyToPath(key);
		return getDataByKeyFormAdmin(path);
	}

    public static String getDataByKeyFormAdmin(String kk) {
        try {
            Stat stat = getInstance().exists(kk, true);
            if (stat != null) {
                String znodeValue = null;
                byte[] resultData = getInstance().getData(kk, true, null);
                if (resultData != null) {
                    znodeValue = new String(resultData);
                }
                return znodeValue;
            } else {
                logger.info(">>>>>>>>>> znodeKey[{}] not found.", kk);
            }
        } catch (KeeperException e) {
            e.printStackTrace();
        } catch (InterruptedException e) {
    ...
    public static String getPathDataByKeyAndGroup(String nodeGroup, String nodeKey) {
        String path = keyGroupToPath(nodeGroup,nodeKey);
        return getDataByKeyFormAdmin(path);
    }
	...
	private static Map<String, String> getAllData(){
		...
		try {
			List<String> childKeys = getInstance().getChildren(Environment.CONF_CACHE_DATA_PATH, true);
	...
	public static void main(String[] args) throws InterruptedException, KeeperException {
		...
		getInstance().delete(Environment.CONF_CACHE_DATA_PATH + "/key02", -1);
		getInstance().delete(Environment.CONF_CACHE_DATA_PATH, -1);

	}


    public static Stat setPathDataByKeyByAdmin(String group, String groupKey, String nodeValue) {
        String path = keyGroupToPathByAdmin(groupKey);
        try {
            Stat stat = getInstance().exists(path, true);
            if (stat == null) {
                createWithParent(path);
            ...
            return zooKeeper.setData(path, nodeValue.getBytes(),stat.getVersion());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
